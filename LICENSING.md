...
## PIVOT-04/PIVOT-05 CUDA local quarantine
...
CUDA is disabled by default. The optional `msvc-cuda-release` preset builds
target-machine validation and modular-backend test executables only under the
ignored build tree. Binary dependency inspection shows no dynamic CUDA runtime
DLL: these local targets contain the statically linked runtime selected by
`CUDA::cudart`. PrimeForge therefore excludes all CUDA libraries and executables
from release packaging. The ordinary
